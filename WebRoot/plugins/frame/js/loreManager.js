 ...
 */
//自定义模块
layui.define(['form','table','comLoreDOM','scrollBar'],function(exports){
	var $ = layui.jquery,form=layui.form,table=layui.table,
		loreDOM = layui.comLoreDOM,comScrollBar=layui.scrollBar;
    var obj = {
    	data : {
    	...
    	switchTiganFun : function(){
    		...
    		$('.switchTiganBtn').on('click',function(){
				...
				$('.switchSureBtn').on('click',function(){
					...
					loreDOM.swithTiGanType(otherInpVal);
			...
			$('.resetBtn').on('click',function(){
				layer.confirm('确认要将此题型再还原至' + _this.data.originTypeTxt + '题型', {
				  ...
				  btn: ['确定','取消'] //按钮
					...
					loreDOM.swithTiGanType(_this.data.originTypeTxt);
					$('.switchTiganBtn').show();
					$('.resetBtn').hide().html('');
					layer.close(index);
				});
